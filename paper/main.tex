...
%% arXiv categories (chosen on the upload form, not by this comment):
...
\title{Sharp Two-Point Escape and Logarithmic Communication\\
for Johnson Products and Sparse Columns}
\author{JinWen Li\\
...
\begin{abstract}
Hambardzumyan, Lovett, and Shirley reformulate the Log-Rank Conjecture as a
relative rectangle problem for two set families whose cross-intersections take
two adjacent values.  We study this formulation when the right family is a
Johnson product or a prescribed sparse subset of a Johnson layer.

Our first result is a globally sharp hypergeometric escape inequality.  For
\(2\le k\le n-2\), a uniform \(B\in\binom{[n]}k\), every \(A\) with
\(2\le |A|\le n-2\), and every integer \(a\),
\[
 \Pr[|A\cap B|\notin\{a,a+1\}]
...
Second, for a product of interior Johnson layers
\(2\le k_i\le n_i-2\), the two-value condition forces every nonconstant row
to be a coordinate function or its complement.  This gives
\[
 \rect(M)\ge\frac1{4(d+1)},\qquad
 \Dcc(M)\le\lceil\log_2(\rank_{\mathbb R}M+1)\rceil+4.
\]
The logarithmic order is optimal up to an additive constant.  At the selector boundary
\(k_i=1\), arbitrary Boolean matrices embed, giving a sharp structural
separation between the interior theorem and the general conjecture.

Third, we obtain logarithmic communication on sparse Johnson columns.
A degree-two unisolvent cross of exactly \(\binom n2\) columns gives
\[
 \Dcc(M)\le\lceil\log_2(\rank_{\mathbb R}M+1)\rceil+1,\qquad
 \rect(M)\ge\frac1{4(\rank_{\mathbb R}M+1)}.
\]
On every \(\beta\)-balanced layer, an anchor-free linear-size random family
retains these conclusions after adversarial deletion of a
\(\beta^2/10\)-fraction of all sampled columns.  Deterministically, the same
bounds follow from an empirical covariance eigenvalue greater than
\(n/[8(n-2)]\) on \(\mathbf1^\perp\); Sylvester--Hadamard designs provide
explicit families of exactly \(n\) columns for \(n=2^t-1\).

As an auxiliary consequence, rank-pooled query transfer gives
\(O(s^8\log(r+1)+s^{16})\) communication for sufficiently deep degree-\(s\)
slices; the query-depth bound used here is due to
Dafni--Filmus--Lifshitz--Lindzey--Vinyals.  These are restricted-column
results and do not resolve the general Log-Rank Conjecture nor improve the
universal \(O(\sqrt r)\) bound.
\end{abstract}
...
its real rank.  The best general result gives a monochromatic rectangle of
relative density \(2^{-O(\sqrt r)}\) in every rank-\(r\) Boolean matrix
\cite{SudakovTomon,HambardzumyanLovettShirley}.

Hambardzumyan, Lovett, and Shirley showed that the conjecture is equivalent
to a relative rectangle problem for families
\(\cA,\cB\subseteq2^{[d]}\) satisfying
\begin{equation}\label{eq:two-level}
 |A\cap B|\in\{a,a+1\}
 \qquad(A\in\cA,\ B\in\cB)
\end{equation}
for some integer \(a\) \cite{HambardzumyanLovettShirley}.  The associated
Boolean matrix
\begin{equation}\label{eq:matrix}
 M_{A,B}=\1_{\{|A\cap B|=a+1\}}
\end{equation}
has real rank at most \(d+1\).  The equivalence does not itself solve the
problem for any prescribed sparse right family.  This paper identifies three
restricted-column regimes where the two-value condition has substantially
more structure.

\paragraph{Main theorem I: sharp two-point escape.}
For an interior Johnson layer \(2\le k\le n-2\) and every
\(A\) with \(2\le|A|\le n-2\), we prove the globally sharp inequality
\[
 \Pr_{B\in\binom{[n]}k}[|A\cap B|\notin\{a,a+1\}]
 \ge\frac25\left(\frac{\min\{k,n-k\}}n\right)^2.
\]
The unique equality is
\((n,k,|A|,a)=(6,3,3,1)\).  The proof separates the short-side and
low-density ranges and closes the central range with a six-coordinate
all-trace certificate.

\paragraph{Main theorem II: Johnson products and the selector boundary.}
Partition \([d]\) into blocks \(C_i\) of sizes \(n_i\), and let the right
family choose exactly \(k_i\) elements from each block.  In the interior
range \(2\le k_i\le n_i-2\), every nonconstant admissible row is a coordinate
function or its complement.  Consequently
\[
 \rect(M)\ge\frac1{4(d+1)},\qquad
 \Dcc(M)\le\lceil\log_2(\rank_{\R}M+1)\rceil+4.
\]
The protocol is logarithmic in the actual restricted rank, and its order is
optimal.  If \(k_i=1\), the block becomes a selector domain and embeds an
arbitrary Boolean matrix.  Thus the boundary is universal rather than a
minor loss in constants.  The abstract atomic-product theorem packages this
classification-and-rank argument; it is not a separate headline claim.

\paragraph{Main theorem III: sparse Johnson columns.}
A degree-two unisolvent cross of exactly \(\binom n2\) columns certifies
Booleanity of every affine row and preserves all row relations.  Every
adjacent-two-value matrix on this fixed sparse family therefore satisfies,
with \(r=\rank_{\R}M\),
\[
 \Dcc(M)\le\lceil\log_2(r+1)\rceil+1,\qquad
 \rect(M)\ge\frac1{4(r+1)}.
\]
On a \(\beta\)-balanced layer the sharp escape theorem yields
\(O_\beta(n)\) random detector families.  Combining detection with uniform
control of affine supports removes the protected anchor and tolerates
deletion of a \(\beta^2/10\)-fraction of all columns.  A deterministic
alternative assumes the empirical covariance gap
\[
 \lambda>\frac{n}{8(n-2)}
 \quad\text{on }\mathbf1^\perp.
\]
Balanced \(2\)-designs supply such families in a broad range, and the
Sylvester--Hadamard construction gives an explicit, degree-one-minimal family
of \(n\) columns for every \(n=2^t-1\), \(t\ge3\).

\paragraph{Additional and auxiliary results.}
A rank-pooled query theorem converts bounded-answer coordinate decision trees
into rank-sensitive protocols.  Its low-degree consequences use literature
inputs: the sharp fixed-degree junta threshold is due to Filmus, the
polynomial slice query depth under
\(\min(k,n-k)\ge144s^4\) is due to
Dafni--Filmus--Lifshitz--Lindzey--Vinyals, and the cube bounds use
Chiarelli--Hatami--Saks and Midrijanis.  In particular, the Dafni et al.\
depth gives
\[
 \Dcc(M)=O\bigl(s^8\log(r+1)+s^{16}\bigr).
\]
We also record, in an appendix, a spectral--lattice sufficient condition for
one large rectangle and a perturbative rational variant.  This condition is
not a communication theorem: arbitrary recursive restrictions need not
preserve its covariance, tube-radius, or generator-entropy hypotheses.

\paragraph{Claim boundary.}
The paper proves restricted-column theorems, not the general Log-Rank
Conjecture.  It neither approaches a proof for arbitrary Boolean matrices nor
improves the universal \(O(\sqrt r)\) bound.  The \(k_i=1\) embedding explains
why extending the product theorem to the selector boundary would already
contain the general problem.  The machine-verified material covers only the
algebraic and finite-combinatorial cores listed in
\nolinkurl{formal/FORMALIZATION_MANIFEST.md}; it does not formalize
communication protocols, \(\Dcc(M)\), or \(\rect(M)\).

\paragraph{Organization.}
Section~2 gives the rank factorization and two-point variance lemma.
Sections~3--5 contain the three main results in the order above.
Section~6 records rank-pooled query transfer as an auxiliary consequence,
followed by related work, limitations, and the machine-verification boundary.
Standard sampling details, low-degree consequences, and the
spectral--lattice one-rectangle criterion are retained in the appendices.

\section{Preliminaries: two-level matrices, rank, and variance}
...
\section{Main theorem I: the sharp two-point escape bound}

We first isolate the hypergeometric input that drives the sparse-column
results.  A four-coordinate fibre gives a preliminary detector; the short-side,
low-density, and six-coordinate arguments then yield the sharp constant and
its unique equality case.  Put
...
\]
then for every \(A\subseteq[n]\) with \(2\le|A|\le n-2\) and every integer \(a\),
\[
 \Pr_{B\in\binom{[n]}k}[|A\cap B|\notin\{a,a+1\}]
 >
...
Assume \(2\le k\le n-2\).  For every \(A\subseteq[n]\) with
\(2\le|A|\le n-2\) and every integer \(a\),
\[
 \Pr_{B\in\binom{[n]}k}[|A\cap B|\notin\{a,a+1\}]
 \ge\frac25
...
Theorem~\ref{thm:global-sharp-hypergeometric-gap}, every row with
$2\le|A|\le n-2$ escapes every consecutive pair on at least a
$\theta_{n,k}$-fraction of the layer.

\section{Main theorem II: Johnson products and the selector boundary}
...
\subsection{Interior Johnson products}
...
\[
 \rank_{\R}M=d-m+1.
\]
Under the one-party-output convention, append Alice's output bit to obtain
a partition into at most $2^{c+1}$ monochromatic rectangles, each of real
rank at most one.  Hence $c\ge\log_2\rank M-1$.
Theorem~\ref{thm:product} supplies the matching order up to an additive
constant.
\end{proof}
...
\[
 \Dcc(M)=O\bigl(t\log\bigl((r+2)(t+2)\bigr)\bigr).
\]
...
\]
Using $|S|\le2rt$ and the standard binomial-sum estimate gives
$O\bigl(\log(t+2)+t\log(rt+2)\bigr)$ bits, hence
$O\bigl(t\log((r+2)(t+2))\bigr)$ bits.  Bob evaluates
\eqref{eq:sparse-row-rep} from his column.
\end{proof}
...
\subsection{Products of interior multislices}
...
\subsection{Sparsity and separation from affine sections}
...
\section{Main theorem III: sparse Johnson columns}

\subsection{Unisolvent transfer and the explicit quadratic cross}
...
\subsection{Standard sampling inputs}

For completeness, standard constant-leverage matrix Chernoff sampling gives a
$V$-unisolvent set of size $O(d\log(d/\delta))$ for a transitive
$d$-dimensional function space, while projection-DPP volume sampling selects
exactly $d$ columns.  These are standard sampling tools, not separate
contributions; their precise statements and short derivations are collected
in Appendix~\ref{app:sampling-query}.  The explicit cross below is the
deterministic construction used in the quadratic sparse-column theorem.
...
Unisolvence through degree two also lifts every linear relation among the
degree-one rows.  Hence the ambient full-slice matrix has the same real rank
$r$ as the restricted matrix.  Choose $r$ basis rows and let $S$ be the set
of their queried coordinates, so $|S|\le r$.  Every ambient row is a linear
combination of the basis rows and therefore an $S$-junta.  Applying
Lemma~\ref{lem:symmetric-tree} at depth one puts every nonconstant row on a
common coordinate universe $U$ with $|U|\le r+1$.

If $U$ is empty, every row is constant and Alice outputs locally.  Otherwise
Alice uses an arbitrary coordinate of $U$ for a constant row and ignores
Bob's reply.  For a nonconstant row she sends the index of its coordinate in
$U$, Bob returns the membership bit, and Alice applies the complement
determined by her own row.  Thus the worst communication cost is
$\lceil\log_2(r+1)\rceil+1$.

For the rectangle bound, do not use one-party transcripts.  Let $K$ be the
set of coordinates that actually occur in nonconstant rows, group rows by
their coordinate, and add one group for constant rows.  Oriented coordinate
rows indexed by $K$ have rank at least $|K|-1$; a loss can occur only from
the fixed-sum relation when $K=[n]$.  Unless $K=[n]$, $r=n-1$, and the only
a zero constant row is present but no one row is present, there are at most
$r+1$ nonempty groups.
Retain a largest group.  For a constant group, retain the larger constant
value.  For a coordinate group, retain the larger orientation and then the
larger of the two column fibres according to membership in that coordinate.
The row loss is at most $2(r+1)$ and the column loss at most two.

It remains to handle the exceptional configuration.  Put $N=|\cA|$ and let
$z$ be the number of zero rows.  Here $r+1=n$, and each coordinate occurs in
only one orientation; otherwise the constant function belongs to the row
span and the rank rises to $n$.  If $z/N\ge1/(4n)$, the zero rows against all
columns give the desired rectangle.  Otherwise some oriented coordinate row
occurs on more than $(1-1/(4n))/n$ of the rows.  Fixing the larger of its two
column fibres retains at least half the columns and gives density greater
than $1/(4n)$.  Thus in every case
\[
 \rect(M)\ge\frac1{4(r+1)}.
\]
\end{proof}
...
\begin{proof}
For fixed $A$ with $2\le|A|\le n-2$ and fixed $a$, the combined
$\theta_{n,k}$ gap and
...
\]
then, with probability at least $1-\delta$, every $A$ with
$2\le|A|\le n-2$ and every consecutive pair $\{a,a+1\}$ have at least
\[
 \theta_{n,k}N/2
\]
...
\[
 |D|\le\theta_{n,k}N/4,
\]
with retained family $\cY_0\cup(\cY_1\setminus D)$; in particular, the
anchor $\cY_0$ is retained even if the two sampled components overlap.
\end{theorem}
...
$\delta/2$, says that every $A$ with $2\le|A|\le n-2$ and every
consecutive pair has at least
...
Likewise every set $A$ with $2\le|A|\le n-2$ retains a witness outside every consecutive
...
communication bound, and the direct row-type/column-fibre argument in that
theorem gives the stated density.
\end{proof}
...
\section{Rank-pooled query transfer}

This auxiliary mechanism is recorded once and then used as a source of
corollaries.  The decision-tree depth inputs are from the literature:
Filmus gives the sharp fixed-degree junta threshold $2s$ on the slice;
Dafni--Filmus--Lifshitz--Lindzey--Vinyals give depth
$5184s^8$ under $\min(k,n-k)\ge144s^4$; the cube bounds use
Chiarelli--Hatami--Saks and Midrijanis.  None of those degree-to-query
statements is claimed here.
...
Applying the theorem to the published query-depth bounds yields the cube,
slice, bounded-alphabet, and multislice results detailed in
Appendix~\ref{app:sampling-query}.  In particular, the Dafni et al.\ depth
bound gives
\[
 \Dcc(M)=O\bigl(s^8\log(r+1)+s^{16}\bigr)
 \quad\text{when}\quad \min(k,n-k)\ge144s^4.
\]
Filmus's junta theorem gives the qualitative
$\Dcc(M)=O_s(\log(r+1))$ at the sharp threshold
$\min(k,n-k)\ge2s$; the complete cube and the very-deep slice use the
Chiarelli--Hatami--Saks and Midrijanis inputs, and the same transfer applies
to bounded alphabets and multislices.  These are auxiliary consequences, not
...
\cite{SudakovTomon}.  This improvement is specific to the restricted
Johnson-product columns and gives no stronger bound for arbitrary matrices.
...
They continue the classical anti-concentration and inverse
Littlewood--Offord line \cite{Erdos,TaoVu,NguyenVu}.

Theorem~\ref{thm:global-sharp-hypergeometric-gap} instead concerns the
length-one local concentration function
$\max_a\Pr[X\in\{a,a+1\}]$ of one hypergeometric law, with a
normalization uniform over all four parameters.  Hush--Scovel give
deviation-from-the-mean tail bounds for hypergeometric variables
\cite{HushScovel}; these do not optimize an arbitrary adjacent pair or
identify the equality case.  The restricted-intersection results of Sgall
and Keevash--Sudakov optimize absolute products of two set families
\cite{SgallRestricted,KeevashSudakovRestricted}; they do not give a
normalized escape bound inside a prescribed complete $k$-layer.  A targeted
search across these local-concentration and restricted-intersection lines
did not locate the constant $2/5$ theorem or the equality
$(6,3,3,1)$.  We therefore present it as a candidate new sharp inequality,
while leaving final priority confirmation to independent review.
...
The one-block Johnson classification is not new.  Filmus--Ihringer call it
folklore and note that equivalent classifications had appeared repeatedly
under different terminology \cite[Theorem~1.2]{FilmusIhringer}; the reduction
of a Boolean degree-one function on a Cartesian product to one nonconstant
factor is elementary as well.  We claim no novelty for either classification.
The candidate new content of Theorem~\ref{thm:interior-product} is the
assembly for the prescribed Johnson-product right family, the support bound
in terms of the \emph{actual restricted real rank}, and the resulting
logarithmic protocol and relative rectangle.  The $k_i=1$ selector embedding
is an elementary boundary witness, included to delimit rather than enlarge
the novelty claim.
...
and actual-rank protocol are the candidate additions here; the count-one
construction is again a boundary witness rather than a new classification
theorem.
...
These targeted comparisons are not a substitute for an exhaustive
publication-level priority audit.  In particular, the paper does not infer
novelty merely from a difference in association-scheme terminology.
...
\item the definition of a trivial unisolvent kernel, lifting of finite linear
relations, and the $f(f-1)$ Booleanity certificate;
\item the local affine swap-pair lemma, rank-basis support pooling, and support
bounds for binary coordinate decision trees;
\item uniform-layer range/kernel lemmas, the block-dependent product affine
kernel, and the equal-block middle-layer arithmetic in
\texttt{ProductJohnsonArithmetic};
\item the short-side and low-density cleared-polynomial arguments, including
the short-side $k=2$ boundary;
\item nonnegativity of the cleared global six-coordinate polynomial and its
unique zero at $(k,t)=(3,0)$; the six-coordinate fibre counts and the
derivation of the escape probability $\zeta$ remain in the manuscript;
\item the deterministic covariance-gap medium-row contradiction and the
Sylvester--Hadamard threshold.
\end{itemize}
Not formalized are the Chernoff/VC sampling and projection-DPP arguments; the
literature inputs of Filmus, Dafni--Filmus--Lifshitz--Lindzey--Vinyals, and
Midrij\={a}nis; the explicit cross $Y_{\times}$; the complete protocol
wrappers; or statements about $\Dcc(M)$ and $\rect(M)$.  These remain paper
arguments.  The exact coverage boundary is recorded in
...
six-coordinate certificate parameters, 3,060 quadratic-cross actual-rank
protocol row families, 9,241,290 affine evaluations, and six
...
\section{Sampling and detailed rank-pooled consequences}
\label{app:sampling-query}

This appendix retains the standard sampling statements and the detailed
low-degree consequences used by the concise transfer discussion in the main
text.
...
\section{Detailed rank-pooled query consequences}
...
The general communication principle is Theorem~\ref{thm:rank-pooled-query} in the main text.  A coordinate domain
...
\section{Auxiliary spectral--lattice one-rectangle criterion}
...
\subsection{Integer generator entropy}
...
\subsection{The one-rectangle theorem}
...
\subsection{Robust transfer from a nearby rational subspace}
